...
| 설정 | AUC | p값 | 판정 |
|---|---:|---:|---|
| 결함을 모두 포함 (초기 구현) | 0.587 | 0.043 | **유의** |
| 결함을 모두 교정 | 0.481 | 0.931 | 유의하지 않음 |
...
모든 결함은 `core.run_value_model()` 과 `core.get_fundamental_history()` 의 인자로 켜고 끌 수 있습니다.
...
- 다만 Yahoo Finance의 데이터는 시간이 지나면 갱신되므로, **실행 시점에 따라 수치가 달라질 수 있습니다.** 논문의 수치는 2026-09-18 기준입니다.

## 한계

무료 데이터로는 해결할 수 없는 한계가 있으며, 논문 6장에 상세히 기술했습니다. 요약하면:

- 지수에서 탈락한 종목의 약 절반이 조회되지 않으며(인수·합병·사명변경), 그 결측이 무작위가 아님
- 시점 고정(point-in-time) 재무 데이터 부재 (소급 수정 문제)
